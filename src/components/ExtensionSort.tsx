...
const defaultGroups: ExtensionGroup[] = [
  ...
  },
  {
    ...
    icon: Code,
    color: "hsl(180, 70%, 45%)",
    extensions: [".js", ".ts", ".tsx", ".py", ".java", ".html", ".css"],
    folder: "Code",
  },
];
...
export default function ExtensionSort() {
  ...
  const addExtension = (groupId: string) => {
    ...
    setGroups((prev) =>
      prev.map((g) =>
        g.id === groupId
          ? { ...g, extensions: [...g.extensions, newExtension] }
          : g
      )
    );
    setNewExtension("");
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              파일 확장자별로 자동 분류 규칙을 설정하세요
            </p>
          </div>
        </div>

        <motion.button
          className="flex items-center gap-2 px-5 py-2.5 rounded-xl gradient-primary text-primary-foreground font-medium shadow-glow hover:opacity-90"
